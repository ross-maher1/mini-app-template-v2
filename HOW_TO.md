...
## 11. Integrating External UI Output (Figma Make, V0, etc.)

When integrating code generated by Figma Make, V0, or similar tools into this template, the template is the base — external output must conform to it, not the other way around. This section exists because past integrations have failed by allowing external styling to override the template's design system.

### The Problem

External code generators produce complete apps with their own CSS, layout shells, navigation, and styling conventions. When integrating this output into the template, it's easy to accidentally overwrite template files with the external versions, losing critical UI elements like the decorative blob, cream background, content padding, and card styles.

### Rules for Integration

#### 1. Template files are immutable — never rewrite them

Treat all template files as read-only sources of truth. The following files must **never** be overwritten with external versions:

- `src/app/globals.css` — contains `.app-shell`, `.app-blob`, `.app-content`, typography classes, CSS variables
- `src/components/layout/AppShell.tsx` — the layout shell with blob + content + nav
- `src/app/layout.tsx` — root layout with providers and fonts
- All files in `src/components/ui/` — Button, Card, Input, BottomBar, etc.

**Workflow:** Clone the template first, then **add** new files from the external output. Never paste external CSS/layout into template files.

#### 2. Diff against the template after every integration step

After making changes, verify no template content was lost:

```bash
git diff origin/main -- src/app/globals.css
git diff origin/main -- src/components/layout/AppShell.tsx
```

If any template file has lost content compared to `origin/main`, that's a bug. Specifically check for:
- `.app-blob` class in `globals.css` (the decorative gradient)
- `bg-[var(--background)]` on `.app-shell` (the cream background)
- `px-6 pb-28 pt-10` on `.app-content` (content padding)
- `<div className="app-blob" aria-hidden="true" />` in `AppShell.tsx`

#### 3. Commit everything before moving on

Never leave foundational changes (CSS, layout) as uncommitted working directory changes while committing page-level changes. Stage and commit **all** related changes together, or commit foundation changes first. Run `git status` after every commit to confirm nothing was left behind.

#### 4. Verify visually before pushing

Before pushing to trigger a deployment, start the dev server locally and verify at least one page renders with the expected template UI:
- Cream background (`#f7f1ea`)
- Decorative blob gradient at the top
- White semi-transparent cards (`bg-white/85`)
- Correct content padding

A 10-second visual check catches what code review misses.

#### 5. Work from template outward, not from external output inward

The correct workflow:
1. Start with the template repo (clone/branch)
2. Add new route files (`app/gigs/page.tsx`, etc.) as **new files**
3. Add new contexts, types, and data as **new files**
4. Modify **only** `BottomBar.tsx` nav items and `layout.tsx` providers — nothing else in the template
5. Never paste the external output's layout/CSS/shell into template files

#### 6. Treat external output as a data source, not a UI source

Extract **only** from the external output:
- Business logic and state management (contexts, hooks)
- TypeScript types and interfaces
- Mock data or API integration code
- Functional behavior (form handling, filtering, CRUD operations)

All visual presentation comes **exclusively** from the template:
- Backgrounds → cream `#f7f1ea` with `.app-blob` gradient
...
- Navigation → template's `BottomBar` component
- Layout → `<main className="space-y-10">` inside `AppShell`
- Spacing → template's padding and gap patterns

If the external output uses `bg-gradient-to-br from-pink-200 via-purple-200` or any custom backgrounds, those classes must **never** appear in the integrated code.

#### 7. Single atomic commit for the integration

The initial integration should be one commit that includes ALL changes — CSS, layout, pages, components, types, data. Don't split "foundation fixes" from "page restyling" across separate commits where one might get missed.

### Integration Checklist

Before pushing an integration, verify:

- [ ] `globals.css` matches the template (has `.app-blob`, `.app-shell` with `bg-[var(--background)]`, `.app-content` with padding)
- [ ] `AppShell.tsx` has the `<div className="app-blob">` element
- [ ] No page files contain external backgrounds (`bg-gradient-to-br`, `from-pink-`, `via-purple-`, etc.)
- [ ] All pages use `<main className="space-y-10">` as their root element
- [ ] All cards use `rounded-2xl border border-slate-200 bg-white/85` (not custom card styles)
- [ ] Navigation uses template's `BottomBar`, not external nav components
- [ ] `git status` shows no uncommitted changes
- [ ] Dev server renders correctly with cream background and blob
